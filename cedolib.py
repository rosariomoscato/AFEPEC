"""
Estrattore di informazioni su ferie e permessi (Residuo AP", "Maturato", "Goduto", "Saldo) da buste paga in formato PDF.

Giugno 2023
Rosario Moscato
versione 0.0.8

I cedolini "Zucchetti", "SEAC" e "Salciarini" sono gestiti correttamente a partire dall'anno 2019, ma ancora si è in fase di testing.
BUG NOTI: 
- nei documenti Zucchetti quando per FERIE e PERMESSI la coppia omologa sulla colonna (per esempio MATURATO o GODUTI) ha il primo valore (ferie) non valorizzato e il secondo (permessi) con un valore preciso la relativa coppia nel file csv di output sarà valorizzata come (0,0);
- in alcuni casi "Goduti" e "Maturati" sono gestiti male.
"""
...
def salciarini_live(input_file, page_num=0):
  """
  Estraggo le informazioni su Ferie e Permessi dai cedolini Salciarini di Live Tech
...
def salciarini_ds(input_file, page_num=0):
  """
  Estraggo le informazioni su Ferie e Permessi dai cedolini Salciarini di DS Tech
  e restituisco tutto il testo contenuto nel PDF, una lista per le Ferie e una lista per i Permessi.
  """
  with open(input_file, 'rb') as file:
    reader = PyPDF2.PdfReader(file)
    ...
    testo = " ".join(righe)
    riga_ferie = righe[-2][:40]
    riga_ferie = riga_ferie.split()
    #for i in range(10):
      #print(f"RIGA -{i}: {righe[-i]}")
  
    riga_permessi = righe[-4][:40]
    riga_permessi = riga_permessi.split()
...
def salciarini(text, numero_altro_pdf, input_file, outcome, final_result):
  ...
  if "LIVE TECH SRL" in text:
    #print("Salciarini LIVE TECH")
    testo, ferie, permessi = salciarini_live(input_file)
    #print("FERIE",ferie)
    #print("PERMESSI",permessi)
  else:
    #print("Salciarini DS TECH")
    testo, ferie, permessi = salciarini_ds(input_file)
    #print("FERIE",ferie)
    #print("PERMESSI",permessi)
